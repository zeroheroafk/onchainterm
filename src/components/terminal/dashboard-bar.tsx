...
export function DashboardBar() {
  ...
  return (
    <div className="flex items-center justify-center gap-4 border-b border-border/30 bg-secondary/5 px-3 py-1 text-[10px] font-mono shrink-0 overflow-x-auto">
      <div className="flex items-center gap-1">
        ...
        <AnimatedNumber value={stats.totalMarketCap} format={formatCompact} className="text-foreground/90 font-semibold text-[11px]" />
      ...
      <div className="hidden sm:flex items-center gap-1">
        ...
        <AnimatedNumber value={stats.totalVolume} format={formatCompact} className="text-foreground/90 font-semibold text-[11px]" />
      </div>
      ...
      <div className="flex items-center gap-1">
        ...
        <AnimatedNumber value={stats.btcDominance} format={(v) => `${v.toFixed(1)}%`} className="text-foreground/90 font-bold text-[11px]" />
      </div>
      ...
      {fearGreed && (
        <>
          ...
          <div className="hidden sm:flex items-center gap-1">
            ...
            <span className={`font-bold text-[11px] ${fgColor(fearGreed.value)}`}>{fearGreed.value}</span>
